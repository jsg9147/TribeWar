...
public class SingleEntityManager : MonoBehaviour
{
    ...
    public void Attack(int attackerID, int defenderID, bool isMine)
    {
        ...
        attacker = isMine ? playerEntities.Find(x => x.id == attackerID) : opponentEntities.Find(x => x.id == defenderID);
        defender = isMine ? opponentEntities.Find(x => x.id == defenderID) : playerEntities.Find(x => x.id == attackerID);

        int attackerBP = attacker.GetEffectiveValue("bp");
        int defenderBP = defender.GetEffectiveValue("bp");

        if (defender.card.role == AttackType.shooter)
